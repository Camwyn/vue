 ...
 */
function i18n() {
	load_theme_textdomain( 'vwp', VWP_PATH . '/languages' );
 }
...
function scripts() {
	 ...
	 *
	 * @param bool vwp_script_debug
	 */
	$debug = apply_filters( 'vwp_script_debug', false );
	$min = ( $debug || defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';

	wp_enqueue_script(
		'vwp',
		VWP_TEMPLATE_URL . "/assets/js/vwp{$min}.js",
		array(),
		VWP_VERSION,
		true
	);
...
function styles() {
	 ...
	 * Flag whether to enable loading uncompressed/debugging assets. Default false.
	 *
	 * @param bool vwp_style_debug
	 */
	$debug = apply_filters( 'vwp_style_debug', false );
	$min = ( $debug || defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';

	wp_enqueue_style(
		'vwp',
		VWP_URL . "/assets/css/vue{$min}.css",
		array(),
		VWP_VERSION
	);
}
...
function header_meta() {
	 ...
	 */
	$humanstxt = apply_filters( 'vwp_humans', VWP_TEMPLATE_URL . '/humans.txt' );
